...
while not done:
    ...
    choice = input("pick a number on what you want to buy ")

    try:
        choice = int(choice)
        if choice in [0,1,2]:
            cart.append(best_buy_items[choice])
            print("you have added:", best_buy_items[choice]["name"])
        else:
            print("invalid please try again ")
            continue
    except ValueError:
        print("not a number, type in 0, 1, or 2")
        continue
    again = input("buy another item? yes/no ")
    if again == "no":
        done = True
